...
const App: React.FC = () => {
  ...
  const handleUpdateRecord = async (table: string, record: any, stateSetter: React.Dispatch<React.SetStateAction<any[]>>) => {
    if (cloudEnabled) {
      try {
        ...
        alert(`Cloud Error: ${e.message || 'Verification failed. Please check your SQL schema.'}`);
      }
    } else {
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-900 gap-4">
      ...
      <p className="font-black text-blue-400 uppercase tracking-widest text-[10px]">Cloud Syncing Academy Data...</p>
    </div>
  );
      ...
      <aside className="hidden md:flex fixed left-0 top-0 bottom-0 w-64 bg-slate-900 text-white flex-col p-8 z-40 border-r border-white/5">
        ...
        <div className="pt-8 border-t border-white/10 mt-auto">
          {user.isLoggedIn ? (
            ...
            <button onClick={() => setShowLoginModal(true)} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black text-[10px] uppercase shadow-lg">Coach Portal</button>
          )}
        </div>
      </aside>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-10 animate-in fade-in duration-500">
             <div className="relative rounded-[48px] overflow-hidden bg-slate-900 text-white shadow-2xl p-10 min-h-[300px] flex flex-col justify-center border-4 border-white">
                {academyBanner && <img src={academyBanner} className="absolute inset-0 w-full h-full object-cover opacity-40 pointer-events-none" />}
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <section className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm flex flex-col">
                   <div className="flex justify-between items-center mb-8 px-2">
                      <h2 className="text-2xl font-black uppercase">Tournaments</h2>
                   ...
                   <div className="space-y-4">
                      {tournaments.map(t => (
                        <div key={t.id} onClick={() => setEditingTournament(t)} className="p-5 bg-slate-50 rounded-3xl border border-slate-100 flex items-center gap-5 hover:border-blue-200 transition-all cursor-pointer">
                ...
                <section className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm flex flex-col">
                   <div className="flex justify-between items-center mb-8 px-2">
                      <h2 className="text-2xl font-black uppercase">News</h2>
          ...
          <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-2">
              ...
              <div className="flex gap-4 w-full md:w-auto">
                <input type="text" placeholder="Search..." className="bg-white border-2 border-slate-100 px-6 py-4 rounded-3xl text-sm font-bold flex-1 md:w-64 outline-none focus:border-blue-600 shadow-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                {isCoach && <button onClick={() => setSelectedStudent({ id: `s${Date.now()}`, name: '', age: 0, birthday: '', profile_pic: 'https://api.dicebear.com/7.x/avataaars/svg?seed=new', level: SkillLevel.BEGINNER, health_status: HealthStatus.FIT, attendance: [], tournament_ids: [], notes: '' })} className="bg-blue-600 text-white px-8 py-4 rounded-3xl font-black text-xs uppercase shadow-xl hover:bg-blue-700 transition-all">Add Athlete</button>}
              </div>
            </header>
            ...
            <div className="space-y-4">
               {[...(sessions || [])].sort((a,b) => new Date(a.date || '').getTime() - new Date(b.date || '').getTime()).map(s => (
                 <div key={s.id} onClick={() => isCoach && setEditingSession(s)} className="bg-white p-8 rounded-[40px] border border-slate-100 flex flex-col md:flex-row md:items-center gap-6 group cursor-pointer hover:shadow-xl transition-all">
                    <div className="w-16 h-16 bg-blue-600 rounded-2xl flex flex-col items-center justify-center text-white shrink-0 shadow-lg">
                    ...
                    <div className="flex-1">
                       <div className="flex items-center gap-3 mb-1"><h3 className="font-black text-xl text-slate-800 uppercase tracking-tight">{s.title}</h3><span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${s.type === 'Special' ? 'bg-amber-50 text-amber-600 border border-amber-100' : 'bg-blue-50 text-blue-600 border border-blue-100'}`}>{s.type}</span></div>
                    ...
                    <div className="flex flex-wrap gap-2">{(s.target_levels || []).map(lvl => <span key={lvl} className="text-[8px] font-black uppercase tracking-widest text-slate-400 border border-slate-100 px-3 py-1 rounded-full">{lvl}</span>)}</div>
                 </div>
               ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               {(dailyPlans || []).map(p => (
                 <div key={p.id} onClick={() => isCoach && setEditingDailyPlan(p)} className="bg-white p-8 rounded-[48px] border border-slate-100 group cursor-pointer hover:shadow-2xl transition-all flex flex-col h-full">
                    <div className="flex justify-between items-start mb-6"><span className="text-[10px] font-black text-blue-600 uppercase bg-blue-50 px-4 py-1 rounded-full">{p.date}</span><span className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{p.total_duration}</span></div>
        ...
        {activeTab === 'coaches' && (
          <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
            <header className="flex justify-between items-end px-2">
               ...
               {isCoach && <button onClick={() => setIsAddingCoach(true)} className="bg-blue-600 text-white px-8 py-4 rounded-3xl font-black text-xs uppercase shadow-xl hover:bg-blue-700 transition-all">New Staff</button>}
            </header>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
               {(coaches || []).map(c => (
                 <div key={c.id} onClick={() => setSelectedCoach(c)} className="bg-white p-8 rounded-[40px] border border-slate-100 group cursor-pointer hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                    <img src={c.profile_pic} className="w-24 h-24 rounded-3xl mx-auto object-cover border-4 border-slate-50 mb-6 group-hover:scale-110 transition duration-500 shadow-sm" alt={c.name} />
        ...
        {activeTab === 'officers' && (
          <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
            <header className="flex justify-between items-end px-2">
               ...
               {isCoach && <button onClick={() => setIsAddingOfficer(true)} className="bg-blue-600 text-white px-8 py-4 rounded-3xl font-black text-xs uppercase shadow-xl hover:bg-blue-700 transition-all">Add Officer</button>}
            </header>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
               {sortedOfficers.map(off => (
                 <div key={off.id} onClick={() => setSelectedOfficer(off)} className="bg-white p-8 rounded-[40px] border border-slate-100 text-center hover:shadow-2xl hover:-translate-y-2 transition-all group cursor-pointer">
                    <img src={off.profile_pic} className="w-24 h-24 rounded-[32px] mx-auto object-cover border-4 border-slate-50 mb-6 group-hover:scale-110 transition duration-500 shadow-sm" alt="" />
                     ...
                     return (
                      <tr key={s.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="py-5 pl-6 font-black text-slate-800 flex items-center gap-4">
                          <img src={s.profile_pic} className="w-10 h-10 rounded-full border-2 border-white shadow-sm" alt="" />
                          {s.name}
                        </td>
      ...
      {showLoginModal && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
          <div className="bg-white p-12 rounded-[56px] w-full max-w-md shadow-3xl relative animate-in zoom-in duration-500">
            ...
            <h3 className="text-4xl font-black mb-10 text-center uppercase tracking-tighter italic">Coach Portal</h3>
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <button type="submit" className="w-full bg-blue-600 text-white py-6 rounded-[32px] font-black uppercase text-xs shadow-2xl active:scale-95 transition-all">Authorize Profile</button>
            </form>
          </div>
        </div>
      )}
